...
interface NavBarProps {
  ...
  onSignOut?: () => void
  /** Dev-only clock/db controls (shown when the dev-login bypass is enabled). */
  showDev?: boolean
  onAdvanceDay?: () => void
  onResetDb?: () => void
}
...
export default function NavBar({
  ...
  const devChip =
    'cursor-pointer rounded-[3px] border border-dashed border-line px-2 py-1 font-mono text-[11px] tracking-[.1em] whitespace-nowrap uppercase transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent'
  return (
    <nav className="border-line-warm fixed inset-x-0 top-0 z-50 flex h-[60px] items-center gap-5 overflow-x-auto border-b bg-[rgba(250,250,250,0.9)] px-[22px] backdrop-blur-[8px]">
      ...
      <div className="ml-auto flex items-center gap-3">
        {showDev && (
          <>
            <button
              ...
              className={`${devChip} text-muted-2 hover:border-muted-2 hover:text-ink`}
            >
              +1 dia
            </button>
            <button
              type="button"
              onClick={onResetDb}
              className={`${devChip} text-muted-2 hover:border-accent hover:text-accent`}
            >
              reset
            </button>
          </>
        ...
        <button
          type="button"
          onClick={onSignOut}
          className="text-muted-2 hover:text-ink focus-visible:outline-accent cursor-pointer font-mono text-[13px] tracking-[.16em] whitespace-nowrap uppercase transition-colors focus-visible:outline-2 focus-visible:outline-offset-2"
        >
          sair
        </button>
      </div>
